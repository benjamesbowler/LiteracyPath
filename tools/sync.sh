...
#   npm run sync
#
# EVERY EXPANSION IS BRACED AND EVERY STRING IS ASCII. BOTH ARE LOAD-BEARING.
#
# macOS ships bash 3.2. When it parses a variable name inside a double-quoted
# string it stops at the first byte that is not a name character -- and it makes
# that decision byte by byte, with no idea that a multibyte UTF-8 character is
# a single character. So "Merging into $MAIN<ellipsis>" was read as the variable
# "MAIN" plus the ellipsis bytes, which under `set -u` aborted the script with
# "MAIN?: unbound variable" at the exact moment it was about to do the useful
# work: it fetched the bundle and then died before merging.
#
# Braces end the name explicitly, so ${MAIN} is safe next to anything. The
# non-ASCII characters are gone as well, because relying on the braces alone
# means the next person to type an em dash reintroduces the bug.
set -euo pipefail
