...
const outputPath = path.join(outputDir, "maths-audio-issue-review.html");
...
const clipCard = (row, index) => `<article data-role="${escapeHtml(row.role)}">
  <label><input data-review-id="${escapeHtml(row.fingerprint)}" type="checkbox"> <span>Issue confirmed</span></label>
...
const songCard = song => {
  ...
  return `<article data-role="maths_song_instrumental">
    <label><input data-review-id="instrumental:${escapeHtml(song.id)}" type="checkbox"> <span>Issue confirmed</span></label>
    <div><strong>${escapeHtml(song.title)}</strong><small>${song.tempo} BPM · original owned instrumental · Leda lyric guide</small><pre>${escapeHtml(song.lyrics)}</pre></div>
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Maths audio issue review</title>
...
<body><main><header><p>Foundation Maths media issue triage</p><h1>Audio issue review</h1><p>All technically valid clips are accepted until flagged. Use this optional sheet only to confirm a reported exact clip before repair or quarantine.</p><nav><strong id="progress">0 / ${clips.length + mathsSongs.length} issues confirmed</strong><select id="role"><option value="">All roles</option>${[...new Set(clips.map(row => row.role))].map(role => `<option>${escapeHtml(role)}</option>`).join("")}<option>maths_song_instrumental</option></select><button id="export" type="button">Export issue JSON</button><button id="clear" type="button">Clear checks</button></nav></header>
<section><h2>Leda voice · ${escapeHtml(MATHS_LEDA_VOICE)}</h2>${clips.map(clipCard).join("\n")}</section>
<section><h2>Original song instrumentals</h2><p>These are released classroom-chant backing tracks plus Leda lyric guides.</p>${mathsSongs.map(songCard).join("\n")}</section>
</main><script>
...
const update=()=>{const state=Object.fromEntries(boxes.map(box=>[box.dataset.reviewId,box.checked]));localStorage.setItem(key,JSON.stringify(state));document.querySelector('#progress').textContent=boxes.filter(box=>box.checked).length+' / '+boxes.length+' issues confirmed';};
...
document.querySelector('#export').addEventListener('click',()=>{const confirmedIssueIds=boxes.filter(box=>box.checked).map(box=>box.dataset.reviewId);const blob=new Blob([JSON.stringify({schemaVersion:1,voice:'${escapeHtml(MATHS_LEDA_VOICE)}',reviewedAt:new Date().toISOString(),confirmedIssueIds,total:boxes.length},null,2)],{type:'application/json'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='maths-audio-issue-review.json';link.click();URL.revokeObjectURL(link.href);});
</script></body></html>`);

console.log(`Maths issue-review sheet written: ${outputPath} (${clips.length} unique Leda files + ${mathsSongs.length} instrumentals).`);
