...
// generateRocketRunAudio.mjs. Synthesize only four static instructions and five
// exact authored isolated words. Existing recordings stay in their own bank.
...
import { RHYME_AUDIO_SCRIPTS, RHYME_WORD_AUDIO, rhymeInstructionPath } from '../src/components/learn/games/games/rhymePopAudio.js';
import { LEDA_PRODUCTION_VOICE } from '../src/data/ledaProductionVoice.js';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const destination = path.join(root, 'public/audio/rhyme-pop');
...
function sourceMismatch(id = '') {
  return new Error(`Rhyme audio ${id} provenance differs; use a new versioned path instead of overwriting`);
}
...
function inspectAudio(file) {
  ...
  if (!Number.isFinite(durationSeconds) || durationSeconds <= 0 || stream?.codec_name !== 'mp3' ||
    ...
    throw new Error(`Invalid Rhyme recording format or duration: ${path.basename(file)}`);
  }
...
function normalizeMp3(wav, mp3) {
  ...
  if (!Number.isFinite(duration) || duration <= 0) throw new Error('Invalid generated Rhyme duration');
...
export async function runRhymePopAudio({ verify = false, dryRun = false } = {}) {
  const rows = [
    ...Object.entries(RHYME_AUDIO_SCRIPTS).map(([id, text]) => ({ id, text, role: 'instruction', audio: rhymeInstructionPath(id) })),
    ...Object.entries(RHYME_WORD_AUDIO).map(([word, audio]) => ({ id: `word-${word}`, text: word, role: 'isolated_word', audio }))
  ];
  ...
  if (priorBytes !== null) {
    ...
    catch { throw new Error('Invalid Rhyme audio provenance JSON'); }
  ...
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'rhyme-pop-audio-'));
  const installed = [];
  try {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    await runRhymePopAudio({ verify: args.includes('--verify'), dryRun: args.includes('--dry-run') });
  } catch (error) {
