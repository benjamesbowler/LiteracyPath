...
//
// Loads the authored candidate pools used by content operations (via the
// candidate loaders), plus the EL letter list and the EL
...
import {
  ...
  loadHfwAssessmentBankCandidates
...
for (const group of groups) {
  for (const skillId of group.skillIds) {
    const questions = group.id === "hfw"
      ? await loadHfwAssessmentBankCandidates(skillId)
      : await loadAssessmentSkillBankCandidates(skillId);
    collectFromAttempt(
      skillId,
      skillId,
