...
function writeImportedStatus() {
  ...
  const audioDeclaration = sameOrderedValues(audioIds, imageIds)
    ? "export const initialSoundAudioMediaIds = initialSoundImageMediaIds;"
    : `export const initialSoundAudioMediaIds = new Set(${JSON.stringify(audioIds, null, 2)});`;
  const completeDeclaration = sameOrderedValues(completeIds, imageIds)
    ? "export const initialSoundCompleteMediaIds = initialSoundImageMediaIds;"
    : sameOrderedValues(completeIds, audioIds)
      ? "export const initialSoundCompleteMediaIds = initialSoundAudioMediaIds;"
