...
assert.match(dialog, /Login tokens and device identifiers are excluded/);
assert.match(dialog, /Only a[\s\S]*privacy-minimal audit tombstone remains/);
assert.match(dialog, /Data-rights request history/);
assert.match(teacherDashboard, /Export or delete learner data/);
