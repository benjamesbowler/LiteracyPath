...
for (const book of guidedReadingBooks) {
  ...
  const visible = book.active !== false && book.qaStatus === "approved" && !book.teacherPreviewOnly;
  if (!visible) {
    failures.push(`${book.id}: exported book must be approved, active, and available to student readers.`);
    continue;
  }
