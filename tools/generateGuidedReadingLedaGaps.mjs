...
// A visible-line repair can synthesize one exact page without generating
// unrelated corpus or isolated-word gaps. Preserve existing mappings below.
const pageTextIndex = process.argv.indexOf("--page-text");
const requestedPageText = pageTextIndex < 0 ? null : process.argv[pageTextIndex + 1];
if (pageTextIndex >= 0 && !requestedPageText?.trim()) {
  throw new Error("Expected --page-text with an exact current visible page line.");
}
const previousTextIndex = process.argv.indexOf("--previous-page-text");
const previousPageText = previousTextIndex < 0 ? null : process.argv[previousTextIndex + 1];
if (previousTextIndex >= 0 && (!requestedPageText || !previousPageText?.trim())) {
  throw new Error("--previous-page-text requires a previous line and --page-text replacement.");
}
...
if (previousPageText && guidedReadingBooks.some(book => (book.pages || []).some(page =>
  ...
  throw new Error("Previous page text is still used by a live page; its mapping cannot be retired.");
}
...
if (requestedPageText && !pageTexts.has(requestedPageText)) {
  throw new Error("Requested page text is not a current gap-generator page or unresolved live page.");
}
...
].filter(record => !requestedPageText || (
  record.role === "guided_page" && record.lookupText === requestedPageText
)).map(record => {
...
const maps = Object.fromEntries(
  ["guided_page", "isolated_word"].map(role => [
    role,
    Object.fromEntries([
      ...(requestedLevel || requestedPageText ? Object.entries(GUIDED_READING_LEDA_GAPS[role] || {}) : []),
      ...records
...
if (previousPageText) delete maps.guided_page[normalizeLedaAudioText(previousPageText)];
if (!dryRun) {
  const moduleText =
...
const result = {
  ...
  ...(requestedPageText ? { selectedPageText: requestedPageText } : {}),
  voice: voiceName,
  ...
  pageClips: records.filter(record => record.role === "guided_page").length,
  wordClips: records.filter(record => record.role === "isolated_word").length,
