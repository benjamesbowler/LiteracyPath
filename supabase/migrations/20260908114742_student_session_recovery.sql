-- Return the server-owned expiry and allow revocation of only the presented token.
begin;
...
begin
  ...
  values (p_student_id, v_token) returning expires_at into v_expires_at;
...
create or replace function public.student_revoke_session(p_token text)
returns json
language plpgsql
volatile
security definer
...
$$;
revoke all on function public.student_revoke_session(text) from public, anon, authenticated;
grant execute on function public.student_revoke_session(text) to anon, authenticated;
