-- Re-asserted callable-function boundary after teacher email confirmation.
--
-- 20260807110000_teacher_email_confirmation.sql defines a new SECURITY DEFINER
-- function (sync_teacher_account_email_confirmation) and replaces an existing
-- one (admin_set_teacher_account_status). A CREATE OR REPLACE keeps the old
-- grants, but a brand-new function is created with PostgreSQL's default EXECUTE
-- grant to PUBLIC — so without this file the sync trigger function would be
-- callable by anon.
--
-- This is the repository's standing convention: every migration that adds a
-- SECURITY DEFINER function is followed by a boundary migration, and
-- tools/databasePolicyContract.mjs fails the build if one is missing. The body
-- below is the 2026-08-01 boundary re-run verbatim. It is idempotent by
-- construction — it revokes EXECUTE from every prosecdef function in the schema
-- and then grants back only the explicit allowlist — so re-running it is how a
-- newly added function gets locked down rather than something to hand-edit.
