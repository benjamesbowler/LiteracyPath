-- Reassert the complete database API boundary after hardening assessment
-- question reporting and adding the administrator review RPC. PostgreSQL
-- grants EXECUTE on new functions to PUBLIC by default, so every role is
-- revoked first and only the reviewed product surface is restored.
