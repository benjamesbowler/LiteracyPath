-- Re-asserted callable-function boundary after the Class Decodable Press learner export.
--
-- 20260807130000_school_name_search.sql replaces public.list_school_names()
-- with public.search_school_names(text). A new function is created with
-- PostgreSQL's default EXECUTE grant to PUBLIC, so without this file the new
-- one would be callable by roles the allowlist never named, and the dropped
-- one's absence would not be asserted anywhere.
--
-- Standing convention: every migration that adds a SECURITY DEFINER function is
-- followed by a boundary migration, and tools/databasePolicyContract.mjs fails
-- the build if one is missing. The body is the boundary re-run, idempotent by
-- construction — it revokes EXECUTE from every prosecdef function in the schema
-- and then grants back only the explicit allowlist.
...
grant execute on function public.student_save_book_revision(text, uuid, uuid, text, jsonb, jsonb) to anon, authenticated;
grant execute on function public.student_submit_book_revision(text, uuid, uuid) to anon, authenticated;
grant execute on function public.student_read_class_press_library(text) to anon, authenticated;
