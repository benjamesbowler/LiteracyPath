-- Product-owner publication decision: every active Guided Reading book has
-- completed the current manuscript, illustration, exact-audio and listening
-- quality review. Publish the exact 206-book runtime catalogue while retaining
-- fail-closed behavior for any future book that is not in this reviewed set.

begin;

create temporary table guided_reading_release_books on commit drop as
select book_id::text
from (values
  ('gr-a-26'),
  ('gr-a-27'),
  ...
  ('first-facts-level-a-06-animals-can'),
  ('first-facts-level-a-07-bugs'),
  ('first-facts-level-a-08-my-pet'),
  ('first-facts-level-a-09-hot-and-cold'),
  ('first-facts-level-a-10-shapes'),
  ('first-facts-level-a-11-at-the-farm'),
  ('first-facts-level-a-12-in-the-sea'),
  ('first-facts-level-a-13-fruit'),
  ('first-facts-level-a-14-the-tree'),
  ('first-facts-level-a-15-baby-animals'),
  ('first-facts-level-a-16-fast-and-slow'),
  ...
  ('first-facts-a-01-look-at-the-colors'),
  ...
  ('first-facts-a-06-baby-animals'),
  ('first-facts-a-07-animals-on-the-farm'),
  ('first-facts-a-08-animals-in-the-ocean'),
  ('first-facts-a-09-animals-at-night'),
  ('first-facts-a-10-bugs-all-around-us'),
  ('first-facts-a-11-pets-we-love'),
  ('first-facts-a-12-shapes-everywhere'),
  ('first-facts-a-13-big-and-small'),
  ...
  ('first-facts-a-19-day-and-night'),
  ...
  ('moonwood-tales-c-24'),
  ('moonwood-tales-c-25'),
  ('meadow-pals-26-muddys-cool-wall'),
  ('meadow-pals-27-splashys-reed-boat'),
  ('meadow-pals-28-woollys-wool-cloud'),
  ('meadow-pals-29-shys-pond-rings'),
  ('meadow-pals-30-cuddlys-yarn-ball'),
  ('meadow-pals-31-bouncys-hay-lift'),
  ('meadow-pals-32-tinys-giant-berry'),
  ('meadow-pals-33-braves-beetle-bridge'),
  ('meadow-pals-34-grumpys-sun-clock'),
  ('meadow-pals-35-gigglys-round-wheel'),
  ('dino-pals-21-fancys-moonleaf-arch'),
  ('dino-pals-22-shys-sinking-path'),
  ('dino-pals-23-flappys-fern-delivery'),
  ('dino-pals-24-clumsys-steady-bowls'),
  ('dino-pals-25-sneezys-seed-cloud'),
  ('dino-pals-26-sunnys-two-part-picnic'),
  ('dino-pals-27-bossys-three-paths'),
  ('dino-pals-28-honkys-echo-tunnel'),
  ('dino-pals-29-cheekys-shadow-show'),
  ('dino-pals-30-dozy-stops-the-melon'),
  ('moonwood-tales-c-26'),
  ('moonwood-tales-c-27'),
  ('moonwood-tales-c-28'),
  ('moonwood-tales-c-29'),
  ...
  ('moonwood-tales-c-34'),
  ('moonwood-tales-c-35')
) as reviewed_books(book_id);

with approved as (
  select
    book_id,
    'approved'::text as status,
    'Product owner approved release after the complete content, illustration, exact-audio, and listening quality pass on 2026-08-09.'::text as review_note,
    null::uuid as reviewed_by,
    now() as reviewed_at
  from guided_reading_release_books
)
insert into public.guided_reading_book_reviews (
  book_id, status, review_note, reviewed_by, reviewed_at, created_at, updated_at
)
select
  book_id, status, review_note, reviewed_by, reviewed_at, reviewed_at, reviewed_at
from approved
...
where public.guided_reading_book_reviews.status <> 'approved';

do $$
declare
  active_approved integer;
  active_not_approved integer;
begin
  select
    count(*) filter (where review.status = 'approved'),
    count(*) filter (where review.status is distinct from 'approved')
  into active_approved, active_not_approved
  from guided_reading_release_books active
  left join public.guided_reading_book_reviews review using (book_id);

  if active_approved <> 206 or active_not_approved <> 0 then
    raise exception
      'Guided Reading release verification failed: approved %, not approved %',
      active_approved, active_not_approved;
  end if;
end
$$;
