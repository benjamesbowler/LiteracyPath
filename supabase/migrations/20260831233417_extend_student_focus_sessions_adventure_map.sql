-- Extend teacher-controlled Student Sessions with Adventure Map assignments
-- and an explicit end destination. Adventure Map uses one bounded wildcard
-- request. The database snapshots either the teacher's one canonical map space
-- or each member's first unfinished cycle from that learner's own progress row.
--
-- The start and student-read RPC input signatures stay unchanged. The teacher
-- end RPC gains one trailing defaulted argument; its old one-argument function
-- is dropped first so PostgREST exposes one unambiguous, backward-compatible
-- endpoint.
...
alter table public.student_focus_sessions
  add column end_action text not null default 'return_home';

alter table public.student_focus_sessions
  add constraint student_focus_sessions_end_action_check check (
    end_action in ('return_home', 'student_picker')
  );

-- Student devices poll without a direct table grant. The reverse membership
-- index keeps the bounded ended-command lookup on one learner's own history.
create index student_focus_members_student_history_idx
  on public.student_focus_session_members (student_id, session_id);
...
declare
  ...
  v_cycle_number_text text;
  v_space_name text;
  v_space_names text[] := array[
    'Farm Gate',
    'Carrot Patch',
    'Duck Pond',
    'Apple Orchard',
...
begin
  ...
  -- paths take a foreign-key row lock on this class, so this yields one
  -- authoritative roster snapshot instead of trusting a browser roster.
  perform 1
  ...
  elsif p_target = 'adventure_map' then
    -- Adventure Map also uses one wildcard, but the server materializes a
    -- complete cycle assignment for every member. The browser cannot supply a
    -- per-student claim about where another learner currently is.
    v_config := p_assignments -> '*';
    if (select count(*) from jsonb_object_keys(p_assignments)) <> 1
      or v_config is null
      or jsonb_typeof(v_config) <> 'object'
      or octet_length(v_config::text) > 4096
      or jsonb_typeof(v_config -> 'map_mode') is distinct from 'string'
      or exists (
        select 1
        from jsonb_object_keys(v_config) as config_keys(config_key)
        where config_key not in ('map_mode', 'cycle_id', 'cycle_number', 'space_name')
      )
    then
      return json_build_object('ok', false, 'error', 'invalid_adventure_assignment');
    end if;

    v_map_mode := btrim(v_config ->> 'map_mode');
    if v_map_mode = 'each_child_current' then
      if (select count(*) from jsonb_object_keys(v_config)) <> 1 then
        return json_build_object('ok', false, 'error', 'invalid_adventure_assignment');
      end if;
      v_shared_config := jsonb_build_object('map_mode', 'each_child_current');
    elsif v_map_mode = 'one_space_for_everyone' then
      if jsonb_typeof(v_config -> 'cycle_id') is distinct from 'string'
        or jsonb_typeof(v_config -> 'cycle_number') is distinct from 'number'
        or jsonb_typeof(v_config -> 'space_name') is distinct from 'string'
      then
        return json_build_object('ok', false, 'error', 'invalid_adventure_assignment');
      end if;

      v_cycle_id := btrim(v_config ->> 'cycle_id');
      v_cycle_number_text := v_config ->> 'cycle_number';
      v_space_name := btrim(v_config ->> 'space_name');
      if v_cycle_number_text !~ '^[1-9][0-9]*$'
        or char_length(v_cycle_number_text) > 2
      then
        return json_build_object('ok', false, 'error', 'invalid_adventure_assignment');
      end if;
      v_cycle_number := v_cycle_number_text::integer;

      if v_cycle_number not between 1 and 27
        or v_cycle_id <> 'cycle-' || v_cycle_number::text
        or v_space_name <> v_space_names[v_cycle_number]
      then
        return json_build_object('ok', false, 'error', 'invalid_adventure_assignment');
      end if;

      v_shared_config := jsonb_build_object(
  ...
  -- All payload validation and ownership/conflict checks happen before this
  -- replacement boundary. Member progress is materialized below in this same
  -- transaction, so any insertion failure rolls this replacement back.
  ...
  select
    ...
    case
      ...
      when p_target = 'adventure_map' and v_map_mode = 'each_child_current' then
        jsonb_build_object(
          'map_mode', 'each_child_current',
          'cycle_id', 'cycle-' || current_cycle.cycle_number::text,
          'cycle_number', current_cycle.cycle_number,
          'space_name', v_space_names[current_cycle.cycle_number]
        )
      when p_target = 'adventure_map' then v_shared_config
      else '{}'::jsonb
    end
  from unnest(v_student_ids) requested(id)
  left join public.student_progress progress
    on progress.student_id = requested.id
   and progress.area = 'el_quest'
   and progress.key = '__all__'
   and p_target = 'adventure_map'
   and v_map_mode = 'each_child_current'
  left join lateral (
    select coalesce(
      (
        select candidate.cycle_number
        from generate_series(1, 27) candidate(cycle_number)
        where not coalesce(
          jsonb_typeof(
            progress.payload #> array[
              'cycles',
              'cycle-' || candidate.cycle_number::text,
              'stars'
            ]
          ) = 'number'
          and (
    ...
    on p_target = 'adventure_map' and v_map_mode = 'each_child_current';
...
drop function public.teacher_end_student_focus_session(uuid);

create function public.teacher_end_student_focus_session(
  p_session_id uuid,
  p_end_action text default 'return_home'
)
returns json
language plpgsql volatile security definer
set search_path = public
...
begin
  ...
  if p_end_action is null or p_end_action not in ('return_home', 'student_picker') then
    return json_build_object('ok', false, 'error', 'invalid_end_action');
  end if;
...
revoke all on function public.teacher_end_student_focus_session(uuid, text)
  from public, anon, authenticated;
grant execute on function public.teacher_end_student_focus_session(uuid, text)
  to authenticated;
