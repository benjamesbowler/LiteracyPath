-- Reassert the complete database API boundary after the assessment durability
-- and two-phase learner-deletion migration.
--
-- PostgreSQL grants EXECUTE on newly created functions to PUBLIC by default.
-- This migration must remain after every migration containing SECURITY
-- DEFINER: it removes execution from every API role, then restores only the
-- reviewed RPC surface used by the product.
...
-- Signed-out teacher signup uses a name-only autocomplete. No school ids,
-- account data, classes, or learners are returned by this reviewed RPC.
