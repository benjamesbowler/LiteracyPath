...
-- ─── Make Supabase's API layer notice the new functions immediately ─────────
notify pgrst, 'reload schema';

-- ─── Status report: everything should say true ───────────────────────────────
select
  exists(select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'student_set_password') as password_save_ready,
  exists(select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
         where n.nspname = 'public' and p.proname = 'student_login') as child_login_ready,
  exists(select 1 from information_schema.tables
         where table_schema = 'public' and table_name = 'student_sessions') as sessions_table_ready,
  exists(select 1 from information_schema.tables
         where table_schema = 'public' and table_name = 'student_progress') as progress_table_ready,
  exists(select 1 from information_schema.columns
         where table_schema = 'public' and table_name = 'students'
           and column_name = 'symbol_password') as password_column_ready;
