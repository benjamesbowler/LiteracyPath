...
export function buildFamilyBridgePlan({ cycleNumber, studentName = "Your child", language = "en" }) {
  ...
  const focus = cycle.focusLetters.map(item => item.grapheme || item.spelling).filter(Boolean).join(copy.listJoiner) || copy.fallbackSounds;
  const quickWords = (cycle.highFrequencyWords || []).slice(0, 4);
  const wordText = quickWords.length ? quickWords.join(", ") : copy.fallbackWords;
  ...
  return {
    ...
    printPrivacy: copy.printPrivacy,
    ...
    activities: [
      ...
      { id: "read-talk", day: 4, title: copy.activityTitles[3], direction: copy.activities.book(book?.title || copy.fallbackBook) },
