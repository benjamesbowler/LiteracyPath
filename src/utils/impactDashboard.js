...
export function buildImpactDashboard({ answers = [], studentIds = [], windowDays = 28, now = new Date() } = {}) {
  ...
  const currentAccuracy=comparable.length?comparable.reduce((sum,row)=>sum+accuracy(row.current),0)/comparable.length:null;
  const priorAccuracy=comparable.length?comparable.reduce((sum,row)=>sum+accuracy(row.prior),0)/comparable.length:null;
