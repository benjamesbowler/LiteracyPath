...
// Arcade games with no art/<id>.webp on disk (reel-read's art lives in
// /images/learn-games/reel-read/). Preloading those URLs is a guaranteed 404.
const GAMES_WITHOUT_CARD_ART = new Set(["reel-read"]);
...
export function warmStudentAssets(world) {
  ...
  const run = () => {
    ...
    // Game card art so the arcade grid pops in instantly. Only arcade games
    // show card art, entries with no art file on disk are skipped (they would
    // be guaranteed 404s), and the legacy PNG icons are no longer warmed.
    GAME_LIST.filter(game =>
      (game.surfaces || []).includes("arcade") && !GAMES_WITHOUT_CARD_ART.has(game.id)
