...
export function validatePressBook(book, { assetIds = [], pageCount = 4 } = {}) {
  ...
  const pages = book.pages.map((page, index) => {
    ...
    if (!text || text.length > 240) throw new Error(`Page ${index + 1} needs 1–240 characters.`);
  ...
  return Object.freeze({ title, planner: Object.freeze({ ...(book.planner || {}) }), pages: Object.freeze(pages) });
}
