...
export function createTransferEvidencePayload({mission,itemResults,supportState="none",mechanicRehearsalCompleted}) {
  ...
  return Object.freeze({schemaVersion:2,missionId:mission.id,context:mission.context,targetKey:mission.targetKey,responseFormat:mission.responseFormat,evidenceUnit:mission.evidenceUnit,supportState,mechanicRehearsalCompleted:true,itemResults:Object.freeze(itemResults.map(result=>Object.freeze({itemId:result.itemId,state:result.state,response:result.response,attempts:Math.max(1,Number(result.attempts)||1)}))),contentVersion:mission.contentVersion,purpose:"transfer_separate_from_mastery",masteryEligible:false});
}
