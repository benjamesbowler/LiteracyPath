...
export function WorksheetMarkingPage({ client, instance, students, onClose, onPlanLesson }) {
  ...
  return <main className="worksheet-marking" aria-labelledby="worksheet-marking-title">
    <header><div><p>Paper-to-Progress</p><h1 id="worksheet-marking-title">{instance.title}</h1><span>{learners.length} students · {targets.length} observation targets · {instance.status}</span></div><button type="button" onClick={onClose}>Back to worksheets</button></header>
