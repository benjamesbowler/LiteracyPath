...
export function DecodableBookEditorPage({ client, token, project, onClose, onSaved }) {
  ...
  async function save({ submit = false } = {}) { setBusy(true); setMessage(""); try { const frozen=validatePressBook(book,{assetIds:PRESS_ASSETS.map(asset=>asset.id)}); const result=await saveStudentPressRevision(client,{token,projectId:project.id,bookId,clientEventId:crypto.randomUUID(),book:frozen,validation:{purpose:"writing_support_not_assessment",pages:validation}}); setBookId(result.book_id); setRevisionId(result.revision_id); if(submit){await submitStudentPressRevision(client,{token,bookId:result.book_id,revisionId:result.revision_id});setSubmitted(true);setMessage("Sent to your teacher. This exact version is locked while they review it.");}else setMessage(`Draft revision ${result.revision} saved privately.`); await onSaved?.(); } catch(error){setMessage(error?.message || "Your book could not be saved.");} finally{setBusy(false);} }
