...
export function ReadingPassportPage({ books, records, scopeKey, onClose }) {
  ...
  return <main className="reading-passport" aria-labelledby="reading-passport-title">
    ...
    {passport.stamps.length === 0 ? <section className="passport-empty"><h2>Your first stamp is waiting</h2><p>Finish a book and it will appear here. You never need to take a photo or record your voice.</p></section> : <section className="passport-stamps" aria-labelledby="passport-stamps-title"><h2 id="passport-stamps-title">Book stamps</h2><div>{passport.stamps.map(stamp => {
      const selected = progress.reflections[stamp.bookId]?.reflectionId;
      return <article key={stamp.bookId}><div className="passport-stamp-mark" aria-hidden="true"><span>READ</span><strong>✓</strong></div><div><span>{stamp.type} · Level {stamp.level}</span><h3>{stamp.title}</h3>{stamp.completedAt && <small>Finished {new Date(stamp.completedAt).toLocaleDateString()}</small>}{stamp.buddyTurns > 0 && <small>Read with Leda · {stamp.buddyTurns} turns</small>}<fieldset><legend>What did this book make you think or feel?</legend>{READING_PASSPORT_REFLECTIONS.map(reflection => <button key={reflection.id} type="button" aria-pressed={selected === reflection.id} className={selected === reflection.id ? "is-selected" : ""} onClick={() => reflect(stamp.bookId, stamp.title, reflection.id)}>{reflection.label}</button>)}</fieldset></div></article>;
    })}</div></section>}
  </main>;
}
