...
function Picture({ src, word, onFailure, className = "" }) {
  return <img className={`cycle-picture ${className}`} src={src} alt={word || ""} draggable="false" data-assessment-media-kind="evidence" data-assessment-media-role="picture" onError={onFailure} />;
}

function HearPicture({ choice, disabled, onHear }) {
  return <button type="button" className="cycle-picture-hear" aria-label={`Hear ${choice.label || choice.value}`} disabled={disabled} data-audio-action="replay" onClick={() => onHear?.(choice.audio, choice.label || choice.value)}><SpeakerIcon /></button>;
}
  ...
  return <div className={`cycle-choice-play cycle-choice-play--${round.mechanicId}`}>
    {round.mechanicId === "pictureSound" ? <div className="cycle-sound-sun" aria-label={`Find the ${round.targetGrapheme} sound`}><span>{round.targetGrapheme}</span><SpeakerIcon /></div> :
      <div className={`cycle-picture-model${rhyme ? " cycle-picture-model--rhyme" : ""}`}>
        ...
        <button type="button" className="cycle-picture-hear" aria-label={`Hear ${round.targetWord}`} disabled={disabled} data-audio-action="replay" onClick={() => onHear?.(round.audio, round.targetWord)}><SpeakerIcon /></button>
...
function WordBuild({ round, disabled, onCommit, onHear, onMediaFailure, supportLevel, onStep }) {
  ...
  return <div className="cycle-word-workshop">
    ...
    <div className="cycle-word-train" role="group" aria-label="Word you are building">
      ...
      {expected.map((_, index) => <div key={index} className={`cycle-word-car${index === nextIndex ? " cycle-word-car--next" : ""}${built[index] ? " cycle-word-car--filled" : ""}`} aria-label={`Letter ${index + 1}: ${built[index] || "empty"}`}><span>{changing ? <button type="button" className="cycle-change-letter" aria-label={`Change letter ${index + 1}: ${built[index]}`} aria-pressed={changeSlot === index} disabled={disabled || miss !== null} onClick={() => { setChangeSlot(index); onStep?.(); }}>{built[index]}</button> : built[index] || ""}</span><i /><i /></div>)}
    </div>
    <div className="cycle-letter-bank" role="group" aria-label="Letters to build the word">
      {round.choices.map(choice => <button type="button" key={choice.id || choice.value} className="cycle-letter-block" disabled={disabled || miss !== null || (changing && changeSlot === null)} onClick={() => place(choice)} aria-label={`Add ${choice.label || choice.value}`}><span>{choice.label || choice.value}</span></button>)}
    </div>
  </div>;
}
...
function SoundSort({ round, disabled, feedbackPending, assessment, onCommit, onHear, onMediaFailure, supportLevel, onSubtarget, priorResponses = [] }) {
  ...
  const [initialPlacements] = useState(() => {
    ...
    for (const item of objects.slice(0, -1)) {
      ...
      if (!response) break;
      restored.push({ ...item, bin: response.selected, correct: response.evidence?.objectCorrect === true });
    }
    return restored;
  });
  ...
  useEffect(() => {
    ...
    if (step.advance) {
      setPlacements(previous => [...previous, { ...step.object, bin: step.bin, correct: step.correct }]);
  ...
  return <div className={`cycle-sort-play${round.objects ? " cycle-sort-play--tray" : ""}`}>
    <div className="cycle-sort-tray">
      ...
      <div className={`cycle-sort-package${picked ? " cycle-sort-package--picked" : ""}`}>
        <button type="button" className="cycle-sort-picture" aria-label={`Pick up ${object.word}`} aria-pressed={picked} disabled={disabled || selected !== null} onClick={() => setPicked(true)}
          onPointerDown={event => { if (disabled || event.button !== 0) return; drag.current = { id: event.pointerId, x: event.clientX, y: event.clientY }; event.currentTarget.setPointerCapture(event.pointerId); }}
          onPointerUp={release} onPointerCancel={() => { drag.current = null; }} onLostPointerCapture={() => { drag.current = null; }}>
          <Picture src={object.image} word={object.word} onFailure={onMediaFailure} />
        </button>
        <button type="button" className="cycle-picture-hear" data-audio-action="replay" aria-label={`Hear ${object.word}`} disabled={disabled} onClick={() => onHear?.(object.audio, object.word)}><SpeakerIcon /></button>
      </div>
    </div>
    ...
    <div className="cycle-sort-bins" role="group" aria-label={round.variant === "syllableSort" ? "Word beats" : "Sound baskets"}>
      {round.choices.map(choice => <div className="cycle-sort-bin-wrap" key={choice.id || choice.value}>
        <button type="button" className="cycle-sort-bin" aria-label={`Put ${object.word} in ${choice.label || choice.value}`} aria-pressed={selected === choice.value} data-cycle-bin={String(choice.value)} disabled={disabled || selected !== null} onClick={() => send(choice)}>
          ...
          {placements.some(item => String(item.bin) === String(choice.value)) && <span className="cycle-sorted-pictures">{placements.filter(item => String(item.bin) === String(choice.value)).map(item => <Picture key={item.word} src={item.image} word={item.word} onFailure={onMediaFailure} />)}</span>}
        </button>
...
export function CycleActivityRenderer(props) {
  const { round } = props;
  if (round.mechanicId === "letterTrace") return <CycleTraceActivity {...props} />;
  if (round.mechanicId === "wordBuild" && round.variant !== "wordParts") return <WordBuild {...props} />;
  if (round.mechanicId === "soundSort") return <SoundSort {...props} />;
