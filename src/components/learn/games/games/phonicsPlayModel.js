...
export function wordTargetLayout(width, height, words, time = 0, formation = 0) {
  ...
  return words.map((word, index) => ({
    word,
    x: (index % columns + 0.5) * cellWidth + Math.sin(time * (0.5 + formation % 3 * 0.1) + index * 2.2 + formation) * Math.max(0, (cellWidth - targetWidth - 10) * 0.4),
    y: (Math.floor(index / columns) + 0.5) * cellHeight + Math.sin(time * (0.4 + formation % 2 * 0.15) + index * 1.7 + formation) * Math.max(0, Math.min(18, (cellHeight - targetHeight - 10) * 0.4)),
