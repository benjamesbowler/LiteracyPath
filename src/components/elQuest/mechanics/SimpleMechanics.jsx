...
function HearWord({ word, disabled, onRequestObjectAudio }) {
  return <ActivityButton className="am-simple-hear wa-audio" type="button" disabled={disabled} aria-label={`Hear ${word}`} onClick={() => onRequestObjectAudio?.(word)}><SpeakerHigh size={22} weight="fill" aria-hidden="true" /><span>Hear</span></ActivityButton>;
}
