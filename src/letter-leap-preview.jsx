...
import LetterLeapGame from "./components/learn/games/games/LetterLeapGame.jsx";
...
function Preview() {
  ...
  const handleEngineReady = useCallback(engine => {
    window.__letterLeapEngine = engine;
  }, []);

  useEffect(() => {
    if (soundEnabled) startGameMusic("letter-leap");
    else stopGameMusic();
    ...
    <div style={{ width: "100vw", height: "100dvh", background: "#050716" }}>
      <LetterLeapGame
        ...
        onComplete={(stars, finalScore, total) => {
          window.__letterLeapComplete = { stars, finalScore, total };
        }}
