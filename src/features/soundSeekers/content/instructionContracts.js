...
const contracts = [
  ...
  audible({ instructionId: "contrast-sort-place-token", powerId: SOUND_POWER_IDS.CONTRAST_SORT, phase: "decision", childText: "Put this item with the matching sound.", cue: "phoneme", expectedAction: "place_sound_token", recordsDomain: EVIDENCE_DOMAINS.GRAPHEME_TO_PHONEME }),
  audible({ instructionId: "word-forge-teach", powerId: SOUND_POWER_IDS.WORD_FORGE, phase: "teach", childText: "Listen to the whole word. Build it with graphemes.", cue: "whole_word", expectedAction: "place_grapheme_tile" }),
  ...
  audible({ instructionId: "word-forge-place-tile", powerId: SOUND_POWER_IDS.WORD_FORGE, phase: "decision", childText: "Choose the grapheme for this sound.", cue: "whole_word", expectedAction: "place_grapheme_tile", recordsDomain: EVIDENCE_DOMAINS.WORD_SEGMENTATION_ENCODING }),
  audible({ instructionId: "blend-bridge-teach", powerId: SOUND_POWER_IDS.BLEND_BRIDGE, phase: "teach", childText: "Touch each grapheme. Blend the word.", cue: "grapheme_sequence", expectedAction: "activate_grapheme_sequence" }),
  audible({ instructionId: "blend-bridge-replay", powerId: SOUND_POWER_IDS.BLEND_BRIDGE, phase: "replay", childText: "Hear the graphemes again.", cue: "grapheme_sequence", expectedAction: "replay_cue" }),
  ...
  audible({ instructionId: "memory-delivery-deliver-cue", powerId: SOUND_POWER_IDS.MEMORY_DELIVERY, phase: "decision", childText: "Use the clue to help the right friend.", cue: "whole_word", expectedAction: "deliver_memory_cue", recordsDomain: EVIDENCE_DOMAINS.HEART_WORD_MAPPING }),
...
export function assertInstructionMatchesChallenge(contract, challenge = {}) {
  ...
  if (challenge.expectedAction && challenge.expectedAction !== contract.expectedAction) {
    throw new Error(`${contract.instructionId}: expected action does not match the challenge`);
  }
  if (challenge.recordsDomain && challenge.recordsDomain !== contract.recordsDomain) {
