...
export default function SoundSeekersCampaign({ progressScopeKey, isSoundEnabled = true, onExit, accessibilitySettings = {}, initialStopId = null }) {
  ...
  return <main style={soundSeekersCampaignCssVariables()} className="ss-campaign" data-presentation={mission?(hud.navigationMode==="depth"?"maze":"platform"):"landscape"} data-buttons={showButtons||undefined} data-input-device={inputDevice} data-reduced-motion={reducedMotion||undefined} data-runtime-status={hardBlocked?'error':loadingImages?'loading':'ready'} data-sound-seekers-game aria-label="Sound Seekers adventure" tabIndex={-1}>
    ...
    <header inert={paused || undefined} className="ss-campaign-top"><button onClick={()=>setDialog('map')} aria-label="World map"><CampaignIcon name="map"/></button><div><strong>{mission?mission.title:stage.name}</strong><small>{mission?`${hud.area?.name||'Adventure'} · ${(hud.index||0)+1}/${hud.total||1}`:CAMPAIGN_WORLDS.find(w=>w.id===stage.worldId)?.name}</small></div><button onClick={()=>setDialog('pause')} aria-label="Pause adventure"><CampaignIcon name="pause"/></button></header>
    ...
    {!paused&&<>{(mission||!navigation.quest)&&<div className="ss-campaign-prompt"><span>{hud.beat?.view.direction==='letter-to-sound'?hud.beat.view.target.grapheme:hud.beat?.prompt.text||(navigation.quest?'Find a friend. Choose your adventure.':'Explore the paths and hidden places.')}</span>{mission&&<ReplayButton onReplay={hearInstruction} sound={sound} blocked={audioBlocked} speaking={speaking}/>}</div>}
...
function ReplayButton({onReplay,sound,blocked,speaking}) {
  return <button className="ss-replay" data-audio-replay data-needs-gesture={blocked||undefined} data-speaking={speaking||undefined} onClick={onReplay} aria-label={sound?'Replay instruction':'Turn spoken instructions on'}><CampaignIcon name={sound&&!blocked?'replay':'sound'}/></button>;
}
