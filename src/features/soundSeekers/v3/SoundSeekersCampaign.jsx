...
export default function SoundSeekersCampaign({ progressScopeKey, isSoundEnabled = true, onExit, accessibilitySettings = {}, initialStopId = null }) {
  ...
  const adoptProgress = useCallback(next => {
    ...
    if(cp.beatIndex!==c.index||JSON.stringify(cp.beatState)!==JSON.stringify(c.state)){
      ...
      setHud({objects:sceneRef.current?.getObjects()||[],navigationMode:sceneRef.current?.getPresentation?.(),area:sceneRef.current?.getArea?.(),beat:c.beats[c.index]?publicBeat(c.beats[c.index]):null,beatState:c.state});
    }
    return next;
  },[]);
  ...
  const refreshHud = useCallback(() => {
    const c = controllerRef.current;
    setHud({ objects: sceneRef.current?.getObjects() || [], navigationMode:sceneRef.current?.getPresentation?.(),area:sceneRef.current?.getArea?.(), beat: c?.beats[c.index] ? publicBeat(c.beats[c.index]) : null, beatState: c?.state || null });
  }, []);
  ...
  useEffect(() => {
    ...
    const gamepad=createAdventureGamepad({onInput:(key,value)=>scene.setInput(key,value),onInteract:()=>scene.key('KeyE',true),onPause:()=>setDialog(d=>d==='pause'?null:d||'pause'),onRelease:()=>scene.release()});
  ...
  return <main style={soundSeekersCampaignCssVariables()} className="ss-campaign" data-presentation={mission?(hud.navigationMode==="depth"?"maze":"platform"):"landscape"} data-sound-seekers-game aria-label="Sound Seekers adventure" tabIndex={-1}>
    ...
    <header inert={paused || undefined} className="ss-campaign-top"><button onClick={()=>setDialog('map')} aria-label="World map">Map</button><div><strong>{mission?(hud.area?.name||'Sound Seekers'):stage.name}</strong><small>{mission&&hud.beat?.act?`${hud.beat.act.title} · ${hud.beat.act.index+1}/${hud.beat.act.total}`:CAMPAIGN_WORLDS.find(w=>w.id===stage.worldId)?.name}</small></div><button onClick={()=>setDialog('pause')} aria-label="Pause adventure">Ⅱ</button></header>
