...
export function addCampaignTraversal(room, variant) {
  ...
  const mechanism=['drawbridge','stepping-stones','hoist'][variant%3];
  const route={id,kind:mechanism,x,y,width,switch:{id:`${id}/lever`,x:x+100,y,label:mechanism==='hoist'?'Raise the stepping platforms':'Lower the crossing',role:'mechanism'},opened:false};
