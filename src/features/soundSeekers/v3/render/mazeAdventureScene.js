...
export function createMazeAdventureScene({stage,mission,beats,beatIndex=0,beatState,heroId,position,onAction,onAdvance,onHear,onSound,onSavePosition,reducedMotion=false}){
  ...
  const objects=()=>state?.done?[{id:'leave-room',...(continues?exitPoint:maze.spawn),role:'exit',label:continues?'Next clue':'Leave the maze'}]:[{id:'inspect-clue',...maze.spawn,role:'clue',label:'Hear the clue'},...(beat.view.choices||[]).map((c,i)=>({...c,...maze.destinations[(i+beatIndex)%maze.destinations.length],role:'destination',label:c.label||beat.view.sceneObjects?.find(o=>o.id===c.icon)?.label||'Explore this place',sceneObject:beat.view.sceneObjects?.find(o=>o.id===c.icon)}))];
  ...
  function trigger(o){if(!o||disposed)return;release();activity=true;if(o.role==='exit')onAdvance?.();else if(o.role==='clue')onHear?.(beat.prompt.cues.map(c=>c.src).filter(Boolean),{kind:'clue'});else{exitPoint={x:player.x,y:player.y};onSound?.('place');onAction?.({type:'CHOOSE',choiceId:o.id});}}
      ...
      for(const o of objects())entries.push({y:o.y,draw:()=>{const yy=o.y*.68;
        if(o.role==='exit')drawPuzzleSprite(c,'door',o.x,yy,120);
          ...
          if(beat.view.phase==='delivery'&&appearance.relation&&appearance.landmark){
            ...
            c.setLineDash([7,6]);c.lineWidth=3;c.strokeStyle=P['adventure-5'];c.beginPath();c.ellipse(o.x+offset.x,yy+offset.y,Math.max(20,offset.size*.55),Math.max(10,offset.size*.25),0,0,Math.PI*2);c.stroke();c.setLineDash([]);
