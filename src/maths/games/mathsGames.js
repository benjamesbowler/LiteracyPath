...
function sequenceRound(seed, index) {
  ...
  const visible = new Set(sequence.filter(Number.isFinite));
  const distractors = Array.from({ length: 21 }, (_, value) => value)
    .filter(value => value !== target && !visible.has(value))
    .sort((left, right) => {
      const distance = Math.abs(left - target) - Math.abs(right - target);
      return distance || left - right;
    })
    .slice(0, 2);
  return { id, mechanic: "trail_step", skillId: "F-N-SEQ-20", prompt: "Repair the path. Which number belongs in the gap?", target, options: balancedOptions({ seed, roundId: id, target, distractors, maximum: 20 }), model: { start: sequence.find(value => value !== null), end: [...sequence].reverse().find(value => value !== null), sequence } };
}
