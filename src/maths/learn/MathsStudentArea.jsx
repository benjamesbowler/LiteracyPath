...
export const MATHS_LESSON_STORAGE_PREFIX = "lp-maths-lesson:v2:";
...
function AreaShell({ studentName, progressScopeKey, surfaceId, title, eyebrow, onHome, children }) {
  return <StudentGlassShell studentName={studentName} scopeKey={progressScopeKey} active="maths" tabs={[]} showWallet={false} showGrownUps={false} onHome={onHome}>
    <main className="maths-student-area maths-student-area-v2" data-child-surface={surfaceId}>
...
export function MathsLessonPlayer({ studentName, progressScopeKey, client, studentId, token, assignment = null, onHome }) {
  ...
  const readCheckpoint = selectedSkillId => {
    try {
      const raw = localStorage.getItem(`${MATHS_LESSON_STORAGE_PREFIX}${studentId || progressScopeKey}:${selectedSkillId}`);
      ...
      return { step: Math.max(0, Math.min(6, Number(value?.step) || 0)), modelState: value?.modelState || null, stageFeedback: String(value?.stageFeedback || "") };
  ...
  const stageReady = mathsLessonStageIsReady(skillId, stage, modelState, stageFeedback);
  ...
  const move = next => {
    setStep(next);
    setStageFeedback("");
    persist({ step: next, modelState, stageFeedback: "" });
  };
  const finish = async () => {
  ...
  const selectStageThought = text => { setStageFeedback(text); persist({ step, modelState, stageFeedback: text }); };
...
export function MathsAssessmentPlayer({ studentName, progressScopeKey, client, studentId, token, assessmentSeed = "", assignment = null, onHome }) {
  ...
  const answer = async (response, renderedResponse = {}) => {
    ...
    window.setTimeout(() => { setFeedback(""); setSubmitting(false); setIndex(current => current + 1); }, 650);
  };
  ...
  if (!started) return <AreaShell eyebrow="Short, calm and untimed" onHome={onHome} progressScopeKey={progressScopeKey} studentName={studentName} surfaceId="maths-check" title="Skills check"><section className="maths-check-start" data-child-choices data-child-progress>
    ...
    <div><p className="maths-stage-label">What will happen</p><h2>Show how you see {mathsSkillById[skillId].childLabel.toLowerCase()}</h2><ul><li><GridNine aria-hidden="true" size={22} /> Six maths decisions</li><li><SpeakerHigh aria-hidden="true" size={22} /> Audio for every instruction</li><li><CheckCircle aria-hidden="true" size={22} /> No timer and no score screen</li></ul><label htmlFor="maths-check-skill"><span>Learning goal</span><select disabled={Boolean(assignedSkill)} id="maths-check-skill" onChange={event => { setSkillId(event.target.value); setIndex(0); setResponses([]); }} value={skillId}>{APPROVED_FOUNDATION_SKILL_IDS.map(id => <option key={id} value={id}>{mathsSkillById[id].childLabel}</option>)}</select></label><button className="maths-primary" data-child-emphasis="primary" data-child-emphasis-cue data-child-primary onClick={() => setStarted(true)} type="button">Start the check <ArrowRight aria-hidden="true" size={20} weight="bold" /></button></div>
  </section></AreaShell>;
  return <AreaShell eyebrow={`${mathsSkillById[skillId].childLabel} · decision ${index + 1} of ${items.length}`} onHome={onHome} progressScopeKey={progressScopeKey} studentName={studentName} surfaceId="maths-check" title="Skills check">
    ...
    <section className="maths-check-card maths-check-card-v2"><div className="maths-check-prompt"><p className="maths-instruction">{item.promptText}</p><MathsAudioButton client={client} compact requestId={`assessment:${item.id}:${item.surface.id}:prompt`} token={token} /></div><div className="maths-check-interaction"><AssessmentInteraction disabled={submitting || Boolean(feedback)} item={item} key={item.itemKey} onAnswer={answer} /></div><p aria-live="polite" className="maths-feedback">{submitting && !feedback ? "Saving response…" : feedback}</p></section>
  </AreaShell>;
}
...
export function MathsStoryLibrary({ studentName, progressScopeKey, client, studentId, token, assignment = null, onHome }) {
  ...
  return <AreaShell eyebrow={`${selected.title} · page ${pageIndex + 1} of ${selected.pages.length}`} onHome={onHome} progressScopeKey={progressScopeKey} studentName={studentName} surfaceId="maths-stories" title="Number story"><article className="maths-story-reader maths-story-reader-v2" data-child-choices data-child-progress><div className="maths-story-page-visual"><img alt={page.exactText} src={page.image || selected.coverImage} /><div className={`maths-story-scene is-page-${page.pageNumber}`} data-story={selected.id}><StoryModel focus={focus} model={page.model} /></div><span>Page {pageIndex + 1} of {selected.pages.length}</span></div><div className="maths-story-page-copy"><button className="maths-story-close" onClick={() => assignedStory ? onHome?.() : setSelected(null)} type="button"><ArrowLeft aria-hidden="true" size={18} /> {assignedStory ? "Maths home" : "Story shelf"}</button><div className="maths-story-text"><p>{page.exactText}</p><MathsAudioButton client={client} requestId={`story:${selected.id}:page:${page.pageNumber}`} token={token} /></div>{page.model?.parts?.length > 1 && <div className="maths-story-notice" role="group" aria-label="Notice the maths in this page"><span>Notice the maths</span><button aria-pressed={focus === "whole"} onClick={() => setFocus("whole")} type="button">Whole: {page.model.total}</button><button aria-pressed={focus === "parts"} onClick={() => setFocus("parts")} type="button">Parts: {page.model.parts.join(" and ")}</button></div>}<aside className="maths-story-talk"><strong>Talk together</strong><span>{selected.teacherPrompts[pageIndex % selected.teacherPrompts.length]}</span></aside><div className="maths-player-nav maths-player-nav-v2"><button disabled={pageIndex === 0} onClick={() => { setPageIndex(pageIndex - 1); setFocus(""); }} type="button"><ArrowLeft aria-hidden="true" size={20} /> Previous</button>{pageIndex < selected.pages.length - 1 ? <button className="maths-primary" data-child-emphasis="primary" data-child-emphasis-cue data-child-primary onClick={() => { setPageIndex(pageIndex + 1); setFocus(""); }} type="button">Next page <ArrowRight aria-hidden="true" size={20} /></button> : <button className="maths-primary" data-child-emphasis="primary" data-child-emphasis-cue data-child-primary onClick={finish} type="button">Finish story <CheckCircle aria-hidden="true" size={20} /></button>}</div></div></article></AreaShell>;
}
...
export function MathsArcade({ studentName, progressScopeKey, client, studentId, token, assignment = null, onHome }) {
  ...
  const [assignmentComplete, setAssignmentComplete] = useState(false);
  const { record, saveState } = useMathsEvidence({ client, studentId, token });
  if (!session) return <AreaShell eyebrow="Practice through play" onHome={onHome} progressScopeKey={progressScopeKey} studentName={studentName} surfaceId="maths-arcade" title="Maths Arcade"><section className="maths-arcade-library" data-child-progress><header><div><p className="maths-stage-label">Four different game worlds</p><h2>Choose how you want to think</h2><span>Every world is untimed. Maths—not speed—moves you forward.</span></div><Sparkle aria-hidden="true" size={58} weight="duotone" /></header><div className="maths-game-grid maths-game-grid-v2" data-child-choices>{mathsGames.map((game, gameIndex) => { const art = mathsArcadeArt(game.id); const Icon = art.icon; return <button className={art.className} data-child-emphasis={gameIndex === 0 ? "primary" : "choice"} data-child-primary={gameIndex === 0 ? "" : undefined} data-game={game.id} key={game.id} onClick={() => { setGameId(game.id); setIndex(0); }} type="button"><span className="maths-game-card-world"><Icon aria-hidden="true" size={54} weight="duotone" /><i /><i /><i /></span><span className="maths-game-card-copy"><small>{art.world}</small><strong>{game.title}</strong><em>{game.strapline}</em><b data-child-emphasis-cue={gameIndex === 0 ? "" : undefined}>{gameIndex === 0 ? "Play next" : "Open world"} <ArrowRight aria-hidden="true" size={18} /></b><small>8 decisions · no timer · no lives</small></span></button>; })}</div></section></AreaShell>;
