...
- Sound Seekers uses third-person textured landscapes connected to bounded platform activities and 2.5D clue mazes, with illustrated prop sprites, touch sticks and browser gamepad controls. Initial sound examples now use onset anchors (m → map); related maze clues preserve the room and player position. Its current route is the three-world campaign, with 30 stages, 150 main missions, 60 optional missions and eight playable Pals. The [release Bible](../SOUND_SEEKERS_RELEASE_BIBLE.md) owns current behaviour and evidence boundaries. Local earned opening-land play and synthetic later-land checks are distinct; twenty hours remains a pacing target, not observed playtime. The campaign merge and fixed-search-path migrations were applied to production on 10 September with explicit owner authorization; hosted engine/delta parity and retry idempotence passed. Authenticated learner save round trips remain unverified. Coordinate shared progress files with this task and Arcade audio files with the game-restoration task.
