...
- Sound Seekers implementation is isolated in `.worktrees/sound-seekers-adventure`. Its current route is the three-world campaign, with 30 stages, 150 main missions, 60 optional missions and eight playable Pals. The [release Bible](../SOUND_SEEKERS_RELEASE_BIBLE.md) owns current behaviour and evidence boundaries. Local earned opening-land play and synthetic later-land checks are distinct; twenty hours remains a pacing target, not observed playtime. The campaign merge migration must be applied before releasing the cloud-enabled client; it has not been applied by this task. Coordinate shared progress files with this task and Arcade audio files with the game-restoration task.
