...
Generated 2026-07-29T23:37:48.837Z by `tools/assessmentRebuild/gate.mjs`. Regenerate after any bank change — never edit by hand. Machine-readable copy: `MEDIA_REQUEST.json`.
...
1. **Audio for everything.** Recorded human audio (or approved premium TTS), child-friendly voice, natural full sentences, no isolated phonemes, no robotic notation:
   - **Instruction/prompt audio** for every item (`spokenPrompt`) — including instructions, 869 unique lines.
   - **Sentence audio** (0 lines) and **passage read-aloud** (527 passages).
   - **Word audio** for every choice/target word: 669 unique words.
   - File convention: follow `public/audio/child-mode/` existing layout; wire via `audioPreferenceManifest`.
2. **Images for everything.** 201 unique word/scene slots; 0 have NO existing asset (listed in the JSON with alt text). House style: flat, warm, no embedded text, answer-neutral (an image must never reveal the answer of the item it appears in — see docs/skills-assessment-rebuild/AUTHORING_STANDARDS.md).
