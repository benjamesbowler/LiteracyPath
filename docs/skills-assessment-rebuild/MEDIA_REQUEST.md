...
Generated 2026-07-29T15:20:54.886Z by `tools/assessmentRebuild/gate.mjs`. Regenerate after any bank change — never edit by hand. Machine-readable copy: `MEDIA_REQUEST.json`.
...
1. **Audio for everything.** Recorded human audio (or approved premium TTS), child-friendly voice, natural full sentences, no isolated phonemes, no robotic notation:
   - **Instruction/prompt audio** for every item (`spokenPrompt`) — including instructions, 289 unique lines.
   - **Sentence audio** (0 lines) and **passage read-aloud** (256 passages).
   - **Word audio** for every choice/target word: 264 unique words.
