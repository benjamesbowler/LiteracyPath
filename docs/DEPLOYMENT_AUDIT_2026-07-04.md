...
## Addendum 3 — Content-quality overhaul (worksheets, map quest, books, decks)

1. **Worksheets rebuilt** (`worksheetBuilder.js`): every exercise now comes from the cycle's own content. Letter hunts guarantee exactly 6 findable targets among taught-only letters (previously most cycles had ZERO — the mix was hardcoded); picture start-sound tasks added; word building uses only letters taught by that cycle with picture cues and missing-vowel tasks; sight-word pages get real hand-written cloze sentences (one per HFW word in the curriculum, 66 words) with earlier-cycles-only distractors; "I" is never traced lowercase. New tests GUARANTEE no two cycles print the same content.
2. **Map/place mismatch fixed**: poems + Present decks rotated worlds every 3 cycles while the quest map uses 9-cycle lands, so 18 of 27 cycles showed the wrong land's characters. `themeWorldForCycle` now equals the map's bands; the 18 wrong-land poems were rewritten (same findWords, same quality bar, correct casts); a new test enforces every poem stars its own land's characters and no other land's. Poem narration was already gated on unrecorded v2 files, so no audio mismatch shipped; `docs/KIMI_GOLD_VOICE_PRESENT_POEMS.md` regenerated with the new texts.
3. **Present decks rebuilt** (`presentationBuilder.js`): decks now teach each cycle's ACTUAL phonemic-awareness skills (compound/syllable/onset/rime deletion, initial/rime substitution, rhyme identify/produce — routed from the curriculum data) with different examples per cycle; a child-goal slide ("I can hear /ă/ and /m/") and per-letter articulation tips were added from the curriculum; fluency decks drill that cycle's own pattern (25 -ay, 26 -y, 27 review); long grapheme lists no longer render as "ng and ang and ing and…" headings. 9 tests cover all 27 cycles.
4. **Quest fluency stations**: Pattern Power now leads with the cycle's own pattern families; Word Chains draw from per-cycle pools.
5. **Guided reading**: full quality + grammar sweep of all 176 live books. The patterned nonfiction ("I have two eyes…") is correct early-reader design and was kept. Two real errors fixed in the audio-free Bob & Nan books ("Nan/Bob pat the pup" → "pats", "Mum lit the big cake" → "the candles"). The genuinely weak legacy books were confirmed already deactivated (students never see them). James & Anna / Aiden & Betty series verified high quality — untouched.
6. Visual previews for eyeballing: `docs/previews/` (3 decks + 5 worksheets).
