...
**Fix:** move pure assessment runtime policy, lazy runtime services, lazy surface definitions, and final guarded route rendering into four owned modules; lazy-load the final surface so it becomes a separate production chunk; keep the public gateway available while stored authentication restores; and add an incremental controller-size ratchet. Milestone 1 reduces `App.jsx` to 6,937 lines without changing the frozen bundle limits.
**Gate:** `check:app-decomposition` fails above the current 7,000-line milestone or when a required boundary disappears. Commit `a681bd56`; `docs/release/artifacts/2026-07-25T06-15-48-440Z/manifest.partial.json` passes the ratchet, zero-warning lint, all 1,262 unit tests, production build, all 12 desktop/mobile smoke journeys, and bundle budgets. The next ratchets remain ≤5,000 and ≤3,000 lines.
