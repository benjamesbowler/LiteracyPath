...
**Date:** 2026-07-25 · **For:** Codex
**Scope: THE ENTIRE PRODUCT.** Every string a human can see: child screens, teacher screens, parent-facing reports, assessment runner, dialogs, toasts, empty states, errors, tooltips, emails, exports, printables, accessible names read by screen readers. The Classes-page rewrite in §6 is the *worked example*, not the scope. Treat any surface whose copy violates this standard as a bug, wherever it lives.
...
2. **One name per concept, product-wide.** Sign-in (never login/access on surfaces) · check (never checkpoint/assessment on teacher surfaces; children never see any of these words) · results (never evidence) · class code · coins · Sparks. If two screens disagree, that's a bug — fix toward this doc.
...
5. **Numbers read as sentences:** "3 of 24", never "3/24" (teacher/parent surfaces). Children get words or pictures, not fractions.
...
## 2. Register A — Children (ages 4–7)

The child may be a pre-reader. Assume every string is **heard** (paired audio) or **decodable/sight-word simple**; never rely on a child reading a word they haven't been taught to decode.

- ≤ 8 words per line; one idea per line; concrete words only ("Tap the sound you hear", not "Select the matching phoneme").
- Buttons say the action a child understands: "Play", "Read to me", "Hear it again", "Try again", "My Hollow".
- **Never negative labels.** "Not yet — try again!" is fine; "Wrong", "Failed", "Needs teaching" must never reach a child's screen.
...
5. Assessment-runner scripts (D-005): every item shows a real task heading + an imperative teacher directive ≥16px ("Say aloud: 'Do *moon* and *spoon* rhyme?' Never show this screen."). "SAY EXACTLY" blocks keep their styling; the directive explains what the child should do.
...
## 6. Worked example — the Classes page, line by line (current → replacement)

### Navigation
| Current | Replace with |
|---|---|
| Today / Classes / Assess / Progress / Plan/Resources / Admin | keep |
| Student Page | **Child preview** (with the "Previewing as…" banner, plan A5.9) |

### Header, school, code, board
| Current | Replace with |
|---|---|
| "Classes — Manage Trial's roster, access, and class settings." | "**Your class** — add children, set up sign-in, and see how Trial is doing." |
| "SchoolBens0 students" | "School: Bens · 0 children" |
| Class code explainer | Keep as-is — "Children enter this on their device to sign in. Keep it inside the classroom." is exactly the right register; match the whole app to it. |
...
| "Class nicknames — Children only see generated Reader nicknames. Class-only is the privacy default." | "Children appear under friendly made-up nicknames — never real names." |
...
| "Create a class — Use the class name your learners already know." | "Create your class — use the name your children know, like 'Willow Class'." |
| "Add or import learners — Use English names or classroom nicknames, never surnames." | "Add your children — first names or nicknames only, no surnames." |
| "Set login pictures — Every learner needs three teacher-set pictures." | "Choose sign-in pictures — each child signs in by tapping their three pictures. You choose them." |
| "Run the first check — One recorded response makes the evidence trail live." | "Do your first check with one child — results appear as soon as you save it." |
| "Continue: Add or import learners" / "Done / Next / Waiting" | "Next: Add your children" / "Done / Next / Later" |

### Sync panel (worst offender)
| Current | Replace with |
|---|---|
| "Evidence delivery" | "**Saving & syncing**" |
| "Learning event sync health" | "Are children's results reaching your dashboard?" |
| "Trial · cumulative delivery from devices seen in the last 7 days" | "Devices used in the last 7 days" |
| "Sync health unavailable — Health telemetry could not be loaded. Learner evidence is unchanged; retry when the connection returns." | "**Can't check right now** — we couldn't reach the server. Nothing is lost: results are safe on the children's devices. **[Try again]**" |
...
| "Started 0/0 ⓘ" | "Have played — 0 of 0" · ⓘ "Children with at least one saved answer, ever." |
| "Learner-weighted (0) Not checked" + "Response-weighted (0) Not checked" | **One tile:** "Class accuracy — not enough results yet". Expanded view still shows both figures (A4.3 unchanged) relabelled "**Averaging children equally**" / "**Averaging every answer equally**", each with counts. |
| "Active today 0/0 ⓘ" | "Played today — 0 of 0" · ⓘ "Any saved activity today, school timezone." |
| "No single class average: 0 of 2 required policy-ready learners; 0% of learners are policy-ready; 70% required." | "A class average appears once at least **2 children** have done enough checks to be measured fairly." · ⓘ carries the exact rule. |

### Groups + bottom
| Current | Replace with |
|---|---|
| "Class groups — Drill down without leaving Trial" | "**Groups** — tap a group to see just those children" |
| "Whole class / Needs attention / Not started / Active today" | "Everyone / **Needs help** / Not started yet / Played today" |
| "Question type guide" (inline on dashboard) | "**What each check measures**" — moved into Help / ⓘ system (A6.3) |
| "Roster administration — Add, import, transfer, archive, or change learner logins. 0 active learners" | "**Manage children** — add, move, or archive children, and change their sign-in. 0 children" |
...
- Reports/exports: "Needs teaching" wording drift → the ONE status vocabulary from plan A4.2, chosen once, used everywhere including CSVs; boilerplate interpretations replaced by the ⓘ contract lines.
- Empty states claiming "in this browser" on cloud-hydrated data → three-beat error/empty pattern.
- Student login errors (A2.6): distinguish "code not found" / "no internet" / "ask your teacher", spoken + illustrated, child register.
...
4. **Sweep order:** Classes (this doc) → Today → Assess runner (D-005/D-007 together) → Progress/Reports → student home + Guided Reading (A1.7) → dialogs/toasts/errors → exports & printables → emails. One commit per surface, each referencing this doc.

*This is not dumbing down. The model, both averages, and the fairness rules stay exactly as built — teachers get a plain sentence, children get words they can hear or decode, parents get plain English, and the ⓘ carries the statistics degree.*
