...
test("D-007 renders plain assessment-window and reading-stage copy without raw route jargon", () => {
  const sessions = [
    makeSession(EL_BENCHMARK_IDS.ENCODING, {
      grade: "K",
      startMicrophase: undefined,
  ...
  sessions.forEach(session => {
    const html = renderAssessment(session);
    assert.match(html, new RegExp(`${session.window === "BOY" ? "Beginning" : session.window === "MOY" ? "Middle" : "End"} of year · starting reading stage:`));
    assert.doesNotMatch(html, />[^<]*(?:BOY:|MOY:|EOY:)[^<]*</);
    assert.doesNotMatch(html, /Kindergarten BOY|Grade [12] (?:BOY|MOY|EOY)/);
    assert.doesNotMatch(html, />[^<]*(?:microphase|passage route|decoding band)[^<]*</i);
  });
...
test("teacher-facing load and save failures never render raw backend error messages", () => {
  assert.match(
    assessmentPageSource,
    /CHECK_LOAD_ERROR_COPY = "We couldn't load this assessment\. Nothing was changed\. Return and choose it again\."/
  );
  assert.match(
    assessmentPageSource,
    /CHECK_SAVE_ERROR_COPY = "We couldn't save this assessment\. Your answers are still on this screen\. Make sure you are connected and try again\."/
  );
  assert.doesNotMatch(assessmentPageSource, /\{planResult\.error\?\.message/);
  assert.doesNotMatch(assessmentPageSource, /The assessment could not be saved: \$\{error\.message\}/);
});
...
test("finish requires a deliberate tally review and counts every terminal outcome", () => {
  ...
  assert.match(assessmentPageSource, /Review the tally before finishing/);
...
test("a completed Decoding word set offers one-tap Finish or Continue with override details hidden", () => {
  ...
  assert.match(stoppingHtml, /aria-label="Item 9, not done yet, locked until the current word group is reviewed"[^>]*disabled/);
...
test("Encoding accepts the suggested starting point in one tap and asks for rationale only on deviation", () => {
  ...
  assert.match(suggestedHtml, /Based on the student’s grade and this time of year/);
...
test("Decoding permits the exact adjacent ceiling suggestion but rejects unrelated out-of-range placement", () => {
  ...
  assert.match(invalidHtml, /saved starting point is outside this assessment/i);
...
test("a valid minute exposes atomic Yes-continue and Not-yet-stop choices", () => {
  ...
  assert.match(html, /Record the teacher accuracy judgment before reviewing what should happen next/);
