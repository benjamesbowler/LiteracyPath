...
test("detects a repeated cross-day medial-vowel pattern", () => {
  const signals = detectMisconceptionSignals({ students: [student], answers: [wrong("pin", "pan", "2026-08-01"), wrong("pin", "pan", "2026-08-02"), wrong("pin", "pan", "2026-08-02")] });
...
test("does not claim a pattern from a single occasion or correct answers", () => {
  ...
  assert.deepEqual(detectMisconceptionSignals({ students: [student], answers }), []);
});
...
test("four repeated same-day errors can surface but remain a hypothesis", () => {
  ...
  const [signal] = detectMisconceptionSignals({ students: [student], answers });
