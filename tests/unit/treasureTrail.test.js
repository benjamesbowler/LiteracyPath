...
const richProgress = {
  ...
  soundSeekers: { trail: { stars: { s1: 3, s2: 1 } } },
...
test("gems add up from every kind of effort", () => {
  ...
  // 6 quest stars + 5 game stars + 4 Sound Seekers stars + 1 story x2 + 2 books = 19
  assert.equal(t.gems, 19);
  assert.deepEqual(t.breakdown, { questStars: 6, gameStars: 5, soundSeekerStars: 4, storiesDone: 1, booksRead: 2 });
});
...
test("the next treasure is always visible and progress is sane", () => {
  ...
  // 19 gems earns the first three prizes on the trail (3, 8, 15).
